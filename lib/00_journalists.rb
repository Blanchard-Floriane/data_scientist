...
def five_string
  arrobase_less = []
  $journalists_list_init.each do |string| 
  arrobase_less << string.delete("@") 
  end
  num_of_5 = 0
...
def uppercase_counting
  ...
  (0..($length_total-1)).each do |i|
    journalist = $journalists_list_init[i]   
    if journalist[1] == journalist[1].upcase
      num_of_uppercase += 1  
    end
  end
